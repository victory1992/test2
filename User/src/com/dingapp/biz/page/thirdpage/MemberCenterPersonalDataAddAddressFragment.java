...
public class MemberCenterPersonalDataAddAddressFragment extends BaseFragment
	 ...
	 * 默认地址
	 */
	private CheckBox ckDefaultAddress;
	/**
	 * 确定
	 */
	...
	public View onCreateView(LayoutInflater inflater,
		...
		return View.inflate(getActivity(), R.layout.member_center_personal_data_add_address,
				null);
	}
	...
	private void initFirstData() {
		...
		if ("true".equals(default_tag)) {
			ckDefaultAddress.setChecked(true);
			isDefault = "true";
		} else {
			ckDefaultAddress.setChecked(false);
			isDefault = "false";
		}
	...
	public void onClick(View v) {
		if (v == ivBack) {
			...
			pro_id = ( (ProWheelAdapter)sex_wheel.getAdapter()).getID(sex_wheel.getCurrentItem());
			city_id = ((CityWheelAdapter)hour_wheel.getAdapter()).getID(hour_wheel.getCurrentItem());
			area_id = ((AreaWheelAdapter)school_wheel.getAdapter()).getID(school_wheel.getCurrentItem());
			String provice = sex_wheel.getAdapter().getItem(
					sex_wheel.getCurrentItem());
			String city = hour_wheel.getAdapter().getItem(
					hour_wheel.getCurrentItem());
			String area = school_wheel.getAdapter().getItem(
					school_wheel.getCurrentItem());
			
//			ProWheelAdapterWheelAdapter adapter = sex_wheel.getAdapter();
...
//			String area = adapter3.getItem(school_wheel.getCurrentItem());
	...
	private void requestEditAddress() {
		...
		DBDao dbDao = new DBDao(getActivity(), AppConstants.DINGAPP);
		if(pro_id == null || pro_id.equals("")){
			pro_id = dbDao.findName(etProvince.getText().toString(), "1");
		}
		if(city_id == null || city_id.equals("")){
			city_id = dbDao.findName(etCity.getText().toString(), "2"); 
		}
		if(area_id == null || area_id.equals("")){
			area_id = dbDao.findName(etArea.getText().toString(), "3");
		}
		postMap.put("province_id", pro_id);
		postMap.put("city_id", city_id);
		postMap.put("county_id", area_id);
		...
		if (address_id != null && !TextUtils.isEmpty(address_id)) {
			...
			StringPostRequest request = new StringPostRequest(postMap,
					AppConstants.ADDRESS_EDIT, editListener, errorListener);
			mQueue.add(request);
//			UIUtil.showToast(getActivity(), "编辑状态");
		} else {
			StringPostRequest request = new StringPostRequest(postMap,
					AppConstants.ADDRESS_ADD, addListener, errorListener);
			mQueue.add(request);
//			UIUtil.showToast(getActivity(), "添加地址");
	...
	private void requestDelAddress() {
		...
		postMap.put("platform", AppConstants.PLATFORM);
		StringPostRequest request = new StringPostRequest(postMap,
				AppConstants.ADDRESS_DELETE, delListener, errorListener);
		mQueue.add(request);
